...
export default {
  ...
  async mounted(){
      try {
        const data = await axios.get('api/balconyFlooring2');
      ...
      try{
        ...
        this.datas = data.data.data.filter(Id => Id.Id == 49);  //NEED TO CHANGE ACCORDING TO ID
  ...
  methods:{
    ...
    async PalleteTextures(){ 
      ...
      let TexturePalletes = this.excludedItems.filter(HouseType => HouseType.HouseType == 2);
      let TextureDefaultPalletes = this.preSelectedItem.filter(HouseType => HouseType.HouseType == 2);
    ...
    async Save(){
     ...
     // ==========================for i-smart==============================
      ...
      data_array.push( 48, this.SmileExcludedItems, this.SmartExcludedItems, this.CubeExcludedItems, this.PalleteExcludedItems, SmileDefault, SmartDefault, CubeDefault, PalleteDefault, this.houseTypes);
      await axios.post('api/saveMaintenanceTool3', data_array ); 

      //data_array.push( 49, this.SmileExcludedItems, this.SmartExcludedItems, this.CubeExcludedItems, SmileDefault, SmartDefault, CubeDefault, this.houseTypes);
      //await axios.post('api/saveMaintenanceTool', data_array ); 
