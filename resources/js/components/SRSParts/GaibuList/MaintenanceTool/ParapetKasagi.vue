        ...
        <v-card-text class="headline">
          ...
          <v-simple-table width="100%" class="grey lighten-5" >
            <thead class="grey lighten-3">
              <tr>
                <th>House Type</th>
                <th>Category Default Selected </th>
                  ...
                  <tr v-if="($PlanData.itemIndexTexture(texturesInAPI, texture.SortNo) == 0)" :key="textureIndex">    
                    ...
                    <td>
                      <v-checkbox 
                        ...
                        color="orange" 
                        hide-details 
                        :disabled="TextureSmile.includes(texture.JapaneseName)"
                        :label="texture.JapaneseName" 
                        :value="texture.JapaneseName"/>
                    </td>
                  </tr>
                  ...
                  <tr v-if="($PlanData.itemIndexTexture(texturesInAPI, texture.SortNo) == 0)" :key="textureIndex">    
                    ...
                    <td>
                      <v-checkbox 
                        ...
                        color="orange" 
                        hide-details 
                        :disabled="TextureSmart.includes(texture.JapaneseName)"
                        :label="texture.JapaneseName" 
                        :value="texture.JapaneseName"/>
                    </td>
                  </tr>
                  ...
                  <tr v-if="($PlanData.itemIndexTexture(texturesInAPI, texture.SortNo) == 0)" :key="textureIndex">    
                    ...
                    <td>
                      <v-checkbox 
                        ...
                        color="orange" 
                        hide-details 
                        :disabled="TextureCube.includes(texture.JapaneseName)"
                        :label="texture.JapaneseName" 
                        :value="texture.JapaneseName"/>
                    </td>
                  </tr>
                ...
                <template v-for="(texture, textureIndex) in texturesInAPI">  
                  <tr v-if="($PlanData.itemIndexTexture(texturesInAPI, texture.SortNo) == 0)" :key="textureIndex">    
                    <td class="secondColumnWidth" :rowspan="texturesInAPI.length">
                      <v-checkbox 
                        class="shrink mt-0" 
                        v-model="textureMaterialPallete" 
                        color="orange" 
                        hide-details 
                        label="アルミニウム" 
                        value="アルミニウム" />   
                    ...
                    <td>
                      <v-checkbox 
                        ...
                        color="orange" 
                        hide-details 
                        :disabled="TexturePallete.includes(texture.JapaneseName)"
                        :label="texture.JapaneseName" 
                        :value="texture.JapaneseName"/>
                    </td>
                  </tr>
...
export default {
  ...
  async mounted(){
      ...
      try {
        const data = await axios.get('api/parapet');
      ...
      try{
        ...
        this.ItemPart = 'パラペット壁笠木';

        //for ヨーロピアンタイル
        this.datas = data.data.data.filter(Id => Id.Id == 2);  //NEED TO CHANGE ACCORDING TO ID

        if( this.datas[0]['excludedItems'] == 0){
            this.excludedItems = JSON.parse('[{"Items":[], "HouseType": "0"}, {"Items": [], "HouseType": "1"}, {"Items": [], "HouseType": "2"}]');
        }else{
            this.excludedItems = JSON.parse(this.datas[0]['excludedItems']);
        }

        if( this.datas[0]['preSelectedItem'] == 0){
            this.preSelectedItem = JSON.parse('[{"HouseType":"0","Material":"", "Item":""},{"HouseType":"1","Material":"", "Item":""}, {"HouseType":"2","Material":"", "Item":""}]');
        }else{
  ...
  methods:{
    ...
    async Save(){
     ...
     // ==========================for i-smart==============================
      ...
      data_array.push( 2, this.SmileExcludedItems, this.SmartExcludedItems, this.CubeExcludedItems, this.PalleteExcludedItems, SmileDefault, SmartDefault, CubeDefault, PalleteDefault, this.houseTypes);
