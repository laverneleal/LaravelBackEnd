<template>
  <v-container class="grey lighten-5 pa-2" fluid v-show="isShown"> 
  <v-simple-table class="tg" width="100%">
    <tr> 
      <td width="38px" class="borderRight" :rowspan="texturesInAPI.length + 3"><center><v-icon @click="toggleSlope()" :color="visibleIcon.color">{{this.visibleIcon.icon}}</v-icon></center></td>
      <td class="firstColumnWidth borderLeft" :rowspan="texturesInAPI.length + 3" >
        <v-label>&nbsp;スロープ(2)</v-label>
      </td>
    </tr>
...
export default {
  ...
  mounted(){
    ...
    bus.$on('Slope2', (data) => {
      this.isShown = data;
      this.$store.commit('deleteHiddenItem', '51')
    })
  ...
  methods:{
    toggleSlope(){
      bus.$emit('Slope2', false);
      bus.$emit('showIconSlope', {icon:"mdi-plus-circle", color: "blue lighten-2"});
