<template>
  ...
  <v-simple-table class="tg" width="100%">
    <tr> 
      <td width="38px" class="borderRight" :rowspan="texturesInAPI.length + 3"><center><v-icon @click="toggleDoma()" :color="visibleIcon.color">{{this.visibleIcon.icon}}</v-icon></center></td>
      <td class="firstColumnWidth borderLeft" :rowspan="texturesInAPI.length + 3" >
        <v-label>&nbsp;玄関内土間(2)</v-label>
      </td>
    </tr>
...
export default {
  ...
  mounted(){
    ...
    bus.$on('Doma2', (data) => {
      this.isShown = data;
      this.$store.commit('deleteHiddenItem', '46')
    })
  ...
  methods:{
    toggleDoma(){
      bus.$emit('Doma2', false);
      bus.$emit('showIconDoma', {icon:"mdi-plus-circle", color: "blue lighten-2"});
